...
class eesg_datasets(DGLDataset):
    ...
    def __init__(self, filePath, funcs_embeddings, strs_embeddings, depth, data_base, mode, args):
        ...
        self.sub_graphs_path = os.path.join(os.path.join(data_base, str(args.fis)),
                                            str(mode) + "_subgraphs" + str(self.max_edge_num) + "_" + str(
                                                self.max_node_num) + str(name_type) + ".bin")
        self.fid_name_path = os.path.join(os.path.join(data_base, str(args.fis)),
                                          str(mode) + "_fid2name" + str(self.max_edge_num) + "_" + str(
                                              self.max_node_num) + str(name_type) + ".json")
        self.func_classes_path = os.path.join(os.path.join(data_base, str(args.fis)),
                                              str(mode) + "_func_classes" + str(self.max_edge_num) + "_" + str(
                                                  self.max_node_num) + str(name_type) + ".json")
        self.subgraphs = {}
        if mode != "predict":
    ...
    def init_func_classes(self):
        ...
        # print(f"Number of valid function names: {len(cop_func)}")
    ...
    def get_subgraph(self, g, funcName):
        ...
        for i in range(self.depth):
            a_nodes, a_edges, all_edges = self.get_next_nodes(g, nodes, len(edges), len(node2id), all_edges, node2id,
                                                              pname)
            nodes = []
        ...
        for cs in tq:
            ...
            for func_index in range(cs_len):
                try:
                    ...
                    print(f"Error in generating subgraph for func_id={func_id}: {e}")
    ...
    def negative_samples_random(self, classes, anchor_func_id, cs, rand=1.0):
        neg_num = int(self.negative_num * rand)
        n_ids = []
        if neg_num == 0:
            for i in range(self.negative_num):
                n_id = self.negative_sample_select(classes, anchor_func_id)
                if n_id in self.subgraphs:  # 检查有效性
                    n_ids.append(n_id)

            return n_ids
        new_cs_set = set(cs)
        anchor_func_embed = self.funcs_embeddings[self.fid2name[str(anchor_func_id)]]
        nns = self.annoy.get_nns_by_vector(anchor_func_embed, 500)
        nns_set = set(nns)
        ans = list(nns_set - nns_set.intersection(new_cs_set))

        same_id = []
        for a in ans:
            if self.fid2name[str(a)].split("|||")[-1] == self.fid2name[str(anchor_func_id)].split("|||")[-1]:
                same_id.append(a)
        ans = list(set(ans) - set(ans).intersection(set(same_id)))
        ans = [n_id for n_id in ans if n_id in self.subgraphs]  # 确保子图存在
        if len(ans) == 0:
            for i in range(self.negative_num):
                n_id = self.negative_sample_select(classes, anchor_func_id)
                n_ids.append(n_id)
            return n_ids
        if len(ans) >= neg_num:
            n_ids = list(ans)[:neg_num]
            self.rand_num += 1
        else:
            n_ids = ans
            self.rand_num += 1
        for i in range(self.negative_num - len(n_ids)):
            n_ids.append(self.negative_sample_select(classes, anchor_func_id))
        return n_ids
        pass

    def negative_sample_select(self, classes, c_id):
        n_ids = np.random.randint(len(classes))
        while (len(classes[n_ids]) == 0) or (c_id in classes[n_ids]) or classes[n_ids][0] not in self.subgraphs or \
                self.fid2name[str(c_id)].split("|||")[-1] == self.fid2name[str(classes[n_ids][0])].split("|||")[-1]:
            n_ids = np.random.randint(len(classes))
    ...
    def init_pairs(self):
        tq = tqdm(self.func_classes, desc="it is init pairs...", position=0, ncols=80)
        sample_num = 0
        for cs in tq:

            # 保留 func_id 在 subgraphs 中的类别
            cs = [func_id for func_id in cs if func_id in self.subgraphs]
            ...
            if (cs_len >= 2):
                for func_index in range(cs_len):
                    ...
                    if flag == 1 and p_id in self.subgraphs:

                        n_ids = self.negative_samples_random(self.func_classes, anchor_func_id, cs,
                                                             rand=self.negative_rand)
                        n_ids = [n_id for n_id in n_ids if n_id in self.subgraphs]  # 确保负样本有效
                        self.a_fids.append(anchor_func_id)
                        self.p_fids.append(p_id)
                        self.n_fids.append(n_ids)
                        sample_num += 1
    ...
    def __getitem__(self, item):
        try:
            ...
            n_ids = self.n_fids[item]
            a_g = self.subgraphs[a_id]
            a_n = self.fid2name[str(a_id)]
            p_g = self.subgraphs[p_id]
            p_n = self.fid2name[str(p_id)]
            n_gs = [self.subgraphs[n_id] for n_id in n_ids]
            n_ns = [self.fid2name[str(n_id)] for n_id in n_ids]
            return a_g, p_g, n_gs, [a_n, p_n, n_ns]
        except KeyError as e:
            print(f"KeyError in __getitem__: Missing subgraph for func_id={e}.")
            raise e
        pass
