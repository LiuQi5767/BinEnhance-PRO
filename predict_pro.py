...
def get_next_nodes(g, ns, edge_num, all_edges, all_nodes, max_edge_num, have_str, rels, pname, funcs_embeddings):
    nodes = []
    edges = []
    for node in ns:
...
def get_subgraph(g, funcName, funcs_embeddings, strs_embeddings, depth, max_edge_num, have_str, relss_type, loop=1):
    ...
    for i in range(depth):
        a_nodes, a_edges, all_edges = get_next_nodes(g, nodes, len(edges), all_edges, node2id, max_edge_num, have_str, relss_type, pname, funcs_embeddings)
        nodes = []
...
def calculate_rtf_similarity(func_name, basic_dict):
...
def get_func_classes(func_embeddings, hsg_path="", MIN_BLOCKS=0):
    ...
    for funcN in cop_func:
        func = funcN.split("_")[0] + "|||" + funcN.split("_")[5] + "|||" + funcN.split("|||")[-1]
        # func = funcN.replace("|||" + funcN.split("|||")[-1], "")
...
def generate_eval_data(classes, poolsize=10000, yarchs="", yoptc="", tarchs="", toptc="", tp="noinline"):
    sample_num = poolsize
...
def eval(model_name, f_strings, global_vars, external_funcs, hsg_data_path, func_embedding_path, str_embedding_path, relss_type, device, args=None,
    ...
    if not os.path.exists(eval_data_path):
        classes = get_func_classes(func_embeddings, MIN_BLOCKS=eval_type, hsg_path=hsg_path)
    ...
    if os.path.exists(savepaths):
        ...
        for fn, embeds in tqdm(func_embeddings.items(), desc="it is init subgraphs..."):
            ...
            sub = get_subgraph(graph, fn, func_embeddings, str_embedding, args.num_layers, args.max_edge_num, have_str=args.have_str, relss_type=relss_type)
            # sub = sub.to(device)
            if sub is None or sub.num_nodes() == 1 or sub.num_edges() == 1:
                continue
            embed = model.forward_once(sub, device=device).cpu().detach().numpy().tolist()
    ...
    for eds in tq:
        ...
        for ed in eds:
            ...
            leng += ed[2]
            predict_num += 1
            sim_s.append(calculate_rtf_similarity(funcNames, f_strings))
            sim_d.append(calculate_rtf_similarity(funcNames, global_vars))
            sim_e.append(calculate_rtf_similarity(funcNames, external_funcs))
        if leng == 0:
            continue
